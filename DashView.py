...
class Screen(Frame):
    def __init__(self, master):
        ...
        self.w = Canvas(master, width=self.canvasWidth,
                        height=self.canvasHeight)
        self.w.grid(row=0, column=0, columnspan=4)

        self.lArrowToggle = Button(master, text="LEFT",
                                   command=self.toggleLeft)
        self.lArrowToggle.grid(row=1, column=0)

        self.sbTotalDistance = Spinbox(master, from_ = 0, to = 100,
                                       increment=5, command=self.toggleTotal)
        self.sbTotalDistance.grid(row=1, column=1)

        self.sbNextDistance = Spinbox(master, from_ = 0.0, to = 5.0,
                                      increment=0.1, command=self.toggleNext)
        self.sbNextDistance.grid(row=1, column=2)

        self.rArrowToggle = Button(master, text="RIGHT",
                                   command=self.toggleRight)
    ...
    def drawLeftArrow(self, x, y, w, h, color="white"):
        self.w.create_polygon(x+w, y-h/2, x, y, x+w, y+h/2, x+w/5, y,
    ...
    def drawTotalDistance(self, x, y, color="white"):
        canvas_id = self.w.create_text(x, y, anchor="nw", fill=color,
                                       font="Helvetica 18")
    ...
    def clearScreen(self):
        color = "black"
        self.w.create_rectangle(0, 0, self.canvasWidth, self.canvasHeight,
                                fill=color, outline=color)
